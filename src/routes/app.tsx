...
const INVITABLE_TEAM_ROLES: TeamRole[] = [
  "procurement_manager", "purchase_approver", "accountant", "sales_manager", "sales_representative",
  "catalog_manager", "inventory_manager", "warehouse_operator", "viewer",
];
    ...
    <div className="mx-auto max-w-7xl px-5 py-8">
      {loading ? <p>Loading account…</p> : !userId ? <section className="border bg-white p-6"><h1 className="text-2xl font-black">{error ? "We could not load your account" : "Sign in to continue"}</h1>{error && <p role="alert" className="mt-3 border border-red-300 bg-red-50 p-3 text-sm text-red-800">{error}</p>}<a className="mt-4 inline-block bg-[#F87908] px-4 py-2 font-bold text-white" href="/auth">{error ? "Return to sign in" : "Sign in"}</a></section> : <>
        ...
        {organization && <>
          ...
          {view === "team" && <section className="mt-6"><h2 className="text-xl font-black">Team ({team.length})</h2><div className="mt-4 grid gap-3">{team.map(member => <Card key={member.user_id}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-bold">{member.email}</p><p className="text-sm text-[#526178]">{teamRoleLabel(member.role)}</p></div>{currentRole === "owner" && member.user_id !== userId && <button disabled={busy} className="button-secondary" onClick={() => void run(() => removeTeamMember(organization.id, member.user_id), "Team member removed.")}>Remove</button>}</div></Card>)}</div>{canManageOrganization && <><form onSubmit={submitInvitation} className="mt-6 grid gap-3 border bg-white p-5 sm:grid-cols-2"><div className="sm:col-span-2"><h3 className="text-lg font-black">Invite a teammate</h3><p className="mt-1 text-sm text-[#526178]">Invite people to this business workspace. FEA operations access is managed separately.</p></div><Text name="email" label="Confirmed work email" type="email" /><Field label="Business role"><select name="role" className={input}>{INVITABLE_TEAM_ROLES.filter(role => organization.kind === "both" || role === "viewer" || role === "accountant" || (buyer && ["procurement_manager", "purchase_approver"].includes(role)) || (seller && ["sales_manager", "sales_representative", "catalog_manager", "inventory_manager", "warehouse_operator"].includes(role))).map(role => <option key={role} value={role}>{teamRoleLabel(role)}</option>)}</select></Field><Action busy={busy}>Create invitation code</Action></form><div className="mt-6"><h3 className="font-black">Invitation codes</h3>{invitations.length === 0 ? <p className="mt-2 text-sm">No invitations yet.</p> : <div className="mt-3 grid gap-2">{invitations.map(item => <Card key={item.id}><p className="text-sm font-bold">{item.email} · {teamRoleLabel(item.role)} · {item.accepted_at ? "accepted" : new Date(item.expires_at) > new Date() ? "pending" : "expired"}</p>{!item.accepted_at && new Date(item.expires_at) > new Date() && <p className="mt-2 break-all font-mono text-sm">{item.token}</p>}</Card>)}</div>}</div></>}</section>}
