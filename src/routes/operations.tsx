...
import {
  getDisputeQueue, getIdentity, getModerationQueue, getOperationsAccess, getVerificationQueue,
  moderateProduct, resolveDispute, reviewVerification, type Dispute, type Product, type VerificationCase,
...
function Operations() {
  ...
  const load = useCallback(async () => {
    ...
    const [nextCases, nextProducts, nextDisputes] = await Promise.all([getVerificationQueue(), getModerationQueue(), getDisputeQueue()]);
    setCases(nextCases); setProducts(nextProducts); setDisputes(nextDisputes); setAccess(true);
  }, []);
...
function DecisionInput({ id, notes, setNotes }: { id: string; notes: Record<string, string>; setNotes: React.Dispatch<React.SetStateAction<Record<string, string>>> }) {
  return <label className="mt-3 grid gap-1 text-sm font-semibold">Decision notes (at least 10 characters)<textarea className="border border-[#C9D3DF] p-2" minLength={10} value={notes[id] ?? ""} onChange={event => setNotes(current => ({ ...current, [id]: event.target.value }))} /></label>;
}
